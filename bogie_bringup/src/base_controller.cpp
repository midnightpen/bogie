...
{
    int SerialCom;
    unsigned char readbuff[20]={0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void ComSetup(const char *COMName)
{
    ...
    cfsetispeed(&options, B38400);   //set recieve bps of serial-com
    cfsetospeed(&options, B38400);   //set send bps of serial-com
...
void *myreadframe_thread(void *pt)
{
    ...
    {   
        while( (myComDev.nread=read(myComDev.SerialCom,&(myComDev.insert_buf),1))>0 ) //get 1byte by 1byte from serial buffer 
        {
            ...
            //data analysis
            if(myComDev.readbuff[0]==0xcc && myComDev.readbuff[1]==0x0A) //top of frame
            {
                //check sum
                ...
                {                
                   // if(myComDev.readbuff[3]==0xff)
                   //     myComDev.readbuff[3]=0;
               //     if(myComDev.readbuff[4]==0xff)
                //        myComDev.readbuff[4]=0;
              //      if(myComDev.readbuff[5]==0xff)
             //           myComDev.readbuff[5]=0;
              //      if(myComDev.readbuff[7]==0xff)
              //          myComDev.readbuff[7]=0;
              //      if(myComDev.readbuff[8]==0xff)
             //           myComDev.readbuff[8]=0;
            //        if(myComDev.readbuff[9]==0xff)
             //           myComDev.readbuff[9]=0;
                  ...
                  myBaseSensorData.delta_encode_right=(myComDev.readbuff[6]>0?1:-1)*((myComDev.readbuff[7]<<16)+(myComDev.readbuff[8]<<8)+myComDev.readbuff[9]);
                    ...
                    //if ((myBaseSensorData.delta_encode_right>-100&&myBaseSensorData.delta_encode_right<100) && (myBaseSensorData.delta_encode_left>-100&&myBaseSensorData.delta_encode_left<100)){
...
{
  ros::init(argc, argv, "bogie_bringupp_node");
  ros::NodeHandle n;

  std::string com_port = "/dev/ttyS5";
